...
export const Navbar: React.FC<Props> = ({
  ...
  const quickTools = [
    ...
    onOpenUserProfile && {
      id: 'user_profile',
      label: isArabic ? 'الملف الشخصي وهوية الطالب وحفظ البيانات' : 'Student Profile & Local Storage',
      desc: isArabic ? 'تسجيل الدخول بـ Google وتخصيص الشعبة والكلية وحفظ البيانات محلياً' : 'Google sign-in, track goals & local device data backup',
      shortcut: '⌥L',
      icon: GraduationCap,
      color: 'text-indigo-400',
      onClick: () => {
          ...
          <div className="flex items-center gap-1 sm:gap-1.5 shrink-0 justify-end min-w-0">
            ...
            {onOpenUserProfile && (
              <button
                ...
                className={`flex items-center gap-1.5 px-2 sm:px-2.5 py-1 rounded-full font-bold text-[11px] transition-all border shadow-xs active:scale-95 shrink-0 cursor-pointer ${
              ...
              >
                ...
                <span className="truncate max-w-[80px] sm:max-w-[120px]">
                  {userProfile?.displayName 
                    ...
                    : isArabic ? 'دخول Google' : 'Google Sign-In'}
                </span>
                {userProfile?.academicTrack && (
                  <span className={`hidden md:inline-block text-[9px] px-1.5 py-0.2 rounded-full font-bold border ${
                    userProfile.academicTrack === 'scientific_science'
                      ? 'bg-sky-500/20 text-sky-300 border-sky-500/40'
                      : userProfile.academicTrack === 'scientific_math'
                      ? 'bg-indigo-500/20 text-indigo-300 border-indigo-500/40'
                      : userProfile.academicTrack === 'stem'
                      ? 'bg-teal-500/20 text-teal-300 border-teal-500/40'
                      : 'bg-amber-500/20 text-amber-300 border-amber-500/40'
                  }`}>
                    {userProfile.academicTrack === 'scientific_science' 
                      ? (isArabic ? 'علمي علوم' : 'Science')
                      : userProfile.academicTrack === 'scientific_math'
                      ? (isArabic ? 'علمي رياضة' : 'Math')
                      : userProfile.academicTrack === 'stem'
                      ? 'STEM'
                      : (isArabic ? 'أدبي' : 'Literary')}
                  </span>
                )}
              </button>
            )}
