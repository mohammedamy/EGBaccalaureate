...
export const InteractiveNormalDistribution: React.FC<Props> = ({ lang, theme = 'dark', isFullscreen = false }) => {
  ...
  return (
    <div className={isFullscreen ? "h-full flex flex-col gap-2 overflow-hidden p-2 sm:p-3" : "space-y-6"}>
      {/* Mode Switcher Tabs */}
      <div className={`flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 ${isFullscreen ? 'pb-2 shrink-0' : 'pb-3'}`}>
        ...
        <div className="flex items-center gap-1 bg-slate-950 p-0.5 sm:p-1 rounded-xl border border-slate-800">
          <button
            onClick={() => setActiveTab('normal_curve')}
            className={`px-2.5 sm:px-3.5 py-1 sm:py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer ${
      ...
      {activeTab === 'normal_curve' && (
        <div className={isFullscreen ? "flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-12 gap-2.5 overflow-hidden" : "space-y-6"}>
          {/* Main SVG Curve Card */}
          <div
            className={`rounded-2xl border relative overflow-hidden flex flex-col justify-between ${
              isFullscreen
                ? 'lg:col-span-8 p-3 min-h-0'
                : 'p-5'
            } ${
              isLight ? 'bg-slate-50 border-slate-200 shadow-sm' : 'bg-slate-950/80 border-slate-800 shadow-xl'
            }`}
          >
            <div className={`flex flex-wrap items-center justify-between gap-2 border-b border-slate-800/80 ${isFullscreen ? 'pb-2 mb-1.5 shrink-0' : 'pb-3 mb-4'}`}>
              ...
              <div className="flex items-center gap-2 bg-emerald-950/60 border border-emerald-500/40 px-3.5 py-1 rounded-xl text-emerald-300 font-mono text-xs font-bold shadow-inner">
            ...
            {/* SVG Plot */}
            <div className={`w-full ${isFullscreen ? 'flex-1 min-h-0 flex items-center justify-center overflow-hidden' : 'overflow-x-auto'}`}>
              <svg
                viewBox={`0 0 ${svgWidth} ${svgHeight}`}
                className={isFullscreen ? "w-full h-full max-h-[320px] select-none object-contain" : "w-full h-auto select-none"}
            ...
            {/* Quick Empirical Rule Presets (inside card when fullscreen) */}
            <div className={`flex flex-wrap items-center gap-2 pt-2 border-t border-slate-800/80 ${isFullscreen ? 'shrink-0' : 'hidden'}`}>
              <span className="text-[11px] font-bold text-slate-400">
                {isArabic ? 'القاعدة التجريبية:' : 'Empirical Rule:'}
              </span>
              <button
                onClick={() => applyEmpiricalRule(1)}
                className="px-2.5 py-1 rounded-lg border border-slate-700 bg-slate-900 hover:bg-slate-800 text-slate-200 text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5"
              >
          ...
          {/* Standalone Quick Empirical Rule Presets (non-fullscreen only) */}
          {!isFullscreen && (
          ...
          <div className={`${
            isFullscreen
              ? 'lg:col-span-4 flex flex-col justify-between gap-2.5 p-3.5 overflow-y-auto'
              : 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 p-5'
          } bg-slate-950/70 rounded-2xl border border-slate-800`}>
            {/* Mean Slider */}
            ...
            {/* Z-Scores telemetry card */}
            <div className={`p-2.5 rounded-xl bg-slate-900/80 border border-slate-800 space-y-1 text-xs ${isFullscreen ? 'block' : 'col-span-full'}`}>
              <span className="font-bold text-slate-400 text-[10px] uppercase block">
                {isArabic ? 'التحويل المعياري (Standard Z-Scores)' : 'Standard Z-Scores'}
              </span>
              <div className="flex items-center justify-between text-slate-300 text-[11px]">
                <span>Z₁ (a) = ({lowerBound} - {mu}) / {sigma} =</span>
                <span className="font-mono font-bold text-amber-400">{zLower.toFixed(2)}</span>
              </div>
              <div className="flex items-center justify-between text-slate-300 text-[11px]">
                <span>Z₂ (b) = ({upperBound} - {mu}) / {sigma} =</span>
                <span className="font-mono font-bold text-sky-400">{zUpper.toFixed(2)}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {activeTab === 'bayes_tree' && (
        <div className={isFullscreen ? "flex-1 min-h-0 flex flex-col gap-2 overflow-hidden" : "space-y-6"}>
          <div className={`bg-slate-950/80 border border-slate-800 rounded-2xl ${isFullscreen ? 'p-3 flex-1 min-h-0 flex flex-col justify-between gap-2.5 overflow-y-auto' : 'p-5 space-y-4'}`}>
            <div className={`flex items-center gap-2 border-b border-slate-800 ${isFullscreen ? 'pb-2 shrink-0' : 'pb-3'}`}>
            ...
            <p className={`text-xs text-slate-300 leading-relaxed ${isFullscreen ? 'shrink-0' : ''}`}>
              {isArabic
            ...
            {/* Tree Diagram Visual */}
            <div className={`grid grid-cols-1 md:grid-cols-2 gap-3 ${isFullscreen ? 'flex-1 min-h-0' : 'pt-2'}`}>
              {/* Branch 1: Condition Positive */}
              <div className={`bg-slate-900/90 border border-indigo-950 rounded-xl ${isFullscreen ? 'p-3' : 'p-4'} space-y-2.5`}>
              ...
              <div className={`bg-slate-900/90 border border-slate-800 rounded-xl ${isFullscreen ? 'p-3' : 'p-4'} space-y-2.5`}>
            ...
            <div className={`bg-gradient-to-r from-emerald-950/50 via-slate-900 to-indigo-950/50 border border-emerald-500/40 rounded-xl ${isFullscreen ? 'p-2.5 shrink-0' : 'p-4'} flex flex-col sm:flex-row items-center justify-between gap-3`}>
            ...
            {/* Bayes Sliders */}
            <div className={`grid grid-cols-1 sm:grid-cols-3 gap-3 ${isFullscreen ? 'pt-1 shrink-0' : 'pt-2'}`}>
              <div className="space-y-1">
                <div className="flex justify-between text-xs font-semibold">
