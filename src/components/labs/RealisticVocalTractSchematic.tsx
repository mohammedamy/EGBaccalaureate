...
export const RealisticVocalTractSchematic: React.FC<RealisticVocalTractProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className={`w-full flex flex-wrap items-center justify-between gap-2.5 pb-3 border-b ${
        isLight ? 'border-indigo-100' : 'border-slate-800/60'
      }`}>
          ...
          <div>
            <h4 className={`text-xs sm:text-sm font-bold tracking-wide flex items-center gap-1.5 ${
              ...
              <span className={`text-[10px] px-2 py-0.5 rounded-full font-mono font-bold ${
                isLight ? 'bg-violet-100 text-violet-900' : 'bg-violet-500/10 text-violet-400'
              }`}>
                HD Anatomy
              </span>
            </h4>
            <div className={`text-[11px] font-serif ${isLight ? 'text-slate-600 font-medium' : 'text-slate-400'}`}>
        ...
        <div className="w-full space-y-2">
          <div className={`text-[11px] font-bold uppercase tracking-wider flex items-center justify-between ${
            ...
            <span className="flex items-center gap-1">
              <Layers className={`w-3.5 h-3.5 ${isLight ? 'text-violet-700' : 'text-violet-400'}`} />
              <span>مواضع النطق التفاعلية (Articulatory Places)</span>
            </span>
            <span className={`text-[10px] font-mono font-bold ${isLight ? 'text-violet-700' : 'text-violet-400'}`}>Click to test anatomy</span>
          </div>
          <div className={`flex flex-wrap gap-1.5 max-h-[140px] overflow-y-auto p-1.5 rounded-2xl border ${
            ...
            {(Object.keys(ARTICULATORY_PLACES) as ArticulatoryPlace[]).map((key) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 py-2 rounded-xl text-xs font-semibold transition-all cursor-pointer flex items-center gap-1.5 min-h-[44px] ${
                    isSelected
                ...
                >
                  ...
                  <span className="text-[10px] opacity-80 font-mono">({cfg.examplePhonemes[0]})</span>
                </button>
              );
            })}
          </div>
      ...
      <div
        className={`w-full p-4 sm:p-5 rounded-2xl border transition-all ${
          isLight ? 'bg-violet-50/90 border-violet-200 text-slate-900 shadow-sm' : 'bg-slate-900/90 border-violet-500/30 text-slate-100'
        }`}
      >
        <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b pb-3 ${
          isLight ? 'border-violet-200' : 'border-slate-800/80'
        }`}>
          <div>
            <div className={`text-xs sm:text-sm font-bold flex items-center gap-1.5 ${
              isLight ? 'text-violet-950 font-black' : 'text-violet-300'
            }`}>
              <span>{name || currentConfig.nameEn}</span>
              {symbol && (
                <span className={`px-2 py-0.5 rounded-lg font-mono text-xs border ${
                  isLight
            ...
            <div className={`text-xs font-serif mt-0.5 font-bold ${
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-violet-600 hover:bg-violet-700 text-white text-xs font-bold transition flex items-center gap-1.5 shadow-md shadow-violet-600/25 cursor-pointer min-h-[44px]"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3 text-xs">
          <div className={`p-3 rounded-xl border ${
            isLight ? 'bg-white border-violet-200 text-slate-900 shadow-xs' : 'bg-slate-950/60 border-slate-800'
          }`}>
            <span className={`font-bold block mb-1 ${isLight ? 'text-slate-700' : 'text-slate-400'}`}>Manner of Articulation:</span>
            <span className={`font-semibold ${isLight ? 'text-slate-950' : 'text-slate-200'}`}>
          ...
          <div className={`p-3 rounded-xl border ${
            ...
            <span className={`font-bold block mb-1 ${isLight ? 'text-slate-700' : 'text-slate-400'}`}>الأصوات الناتجة (IPA):</span>
        ...
        <div className={`mt-3 text-[11px] sm:text-xs leading-relaxed p-3 rounded-xl border ${
          isLight ? 'bg-white border-violet-200 text-slate-800 shadow-xs' : 'bg-slate-950/40 border-slate-800/60 text-slate-300'
        }`}>
          <p className="mb-1 font-medium">{currentConfig.descriptionEn}</p>
          <p className={`font-serif font-medium ${isLight ? 'text-amber-950' : 'text-amber-200/90'}`} dir="rtl">
