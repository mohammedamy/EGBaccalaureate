...
const EXPERIMENTS: GuidedExperiment[] = [
  ...
  },
  {
    ...
    hypothesisEn: 'Induced EMF in a rotating coil is proportional to angular speed omega, turns N, field B, and area A: E = NBA*omega*sin(theta).',
    hypothesisAr: 'تتناسب القوة الدافعة الكهربية المستحثة في ملف يدور طردياً مع السرعة الزاوية omega وعدد اللفات ومساحة الملف وشدة المجال.',
    apparatusEn: ['Rotating armature coil in permanent B field', 'Digital oscilloscope with sine wave display', 'Two continuous bronze slip rings with carbon brushes', 'Split-ring commutator toggle', 'Angle scrubber theta'],
    apparatusAr: ['ملف دينامو قابل للدوران بين قطبي مغناطيس', 'راسم ذبذبات أوسيلوسكوب رقمي لعرض منحنى الجهد', 'حلقتان معدنيتان للانزلاق وفرشاتا كربون', 'مقوم معدني مشقوق لنصفين للتيار المستمر', 'منزلق زاوية الدوران اللحظية'],
    stepsEn: [
      'Set coil turns N = 200, magnetic field B = 0.5 T, and area A = 0.04 m^2.',
      'Rotate armature coil and observe sine waveform on the oscilloscope display.',
      'Record peak voltage E_max when coil plane is parallel to field (theta = 90 deg).',
      'Verify that induced EMF = 0 when coil plane is perpendicular to field lines (theta = 0 deg).',
      'Switch to DC Commutator mode and note current rectification into unidirectional half-waves.',
    ],
    stepsAr: [
      'اضبط عدد اللفات N = ٢٠٠، وكثافة الفيض B = ٠٫٥ تسلا، ومساحة المقطع A = ٠٫٠٤ م².',
      'دوّر ملف الدينامو ولاحظ المنحنى الجيبي المتولد على شاشة الأوسيلوسكوب.',
      'سجّل القيمة العظمى للجهد E_max عندما يكون مستوى الملف موازياً لخطوط الفيض (theta = ٩٠ درجة).',
      'تحقق من انعدام ق.د.ك المستحثة عندما يكون مستوى الملف عمودياً على خطوط الفيض (theta = ٠ درجة).',
  ...
  },
  {
    ...
    hypothesisEn: 'At resonant frequency f0 = 1/(2*pi*sqrt(L*C)), inductive and capacitive reactances cancel (XL = XC), minimizing impedance to Z = R and maximizing current.',
    hypothesisAr: 'عند تردد الرنين f0 = 1/(2*pi*sqrt(L*C))، تتساوى المفاعلتان وتلغي كل منهما الأخرى (XL = XC)، فتصل المعاوقة لأقل قيمة Z = R والتيار لأقصى قيمة.',
    apparatusEn: ['Variable frequency AC power source', 'Inductor coil (L = 100 mH)', 'Capacitor (C = 20 uF)', 'Resistor (R = 50 Ohms)', 'Real-time phasor diagram monitor and resonance response curve'],
    apparatusAr: ['مصدر تيار متردد متغير التردد', 'ملف حث مهمل المقاومة (L = ١٠٠ مللي هنري)', 'مكثف ثابت السعة (C = ٢٠ ميكروفاراد)', 'مقاومة أومية (R = ٥٠ أوم)', 'شاشة متجهات طورية ومنحنى استجابة الرنين'],
    stepsEn: [
      'Set R = 50 Ohms, L = 100 mH, C = 20 uF, and V = 100 V.',
      'Sweep frequency from 20 Hz to 300 Hz and record total impedance Z and current I.',
      'Click "Snap to Resonant f0" (112.5 Hz); observe phase angle phi drop exactly to 0 deg.',
    ...
    stepsAr: [
      'اضبط المقاومة R = ٥٠ أوم، والحث L = ١٠٠ مللي هنري، والسعة C = ٢٠ ميكروفاراد، والجهد V = ١٠٠ فولت.',
      'غيّر التردد تدريجياً من ٢٠ إلى ٣٠٠ هرتز وسجّل قيم المعاوقة Z وشدة التيار I.',
      'اضغط على "ضبط تردد الرنين f0" (١١٢٫٥ هرتز) ولاحظ هبوط زاوية الطور phi إلى صفر تماماً.',
      'تحقق من بلوغ شدة التيار قيمتها العظمى I_max = V / R = ٢٫٠ أمبير عند الرنين.',
  ...
  },
  {
    ...
    hypothesisEn: 'Standard Daniell cell potential E0 = 1.10 V decreases as anode [Zn2+] increases and increases as cathode [Cu2+] increases: E = E0 - (RT/nF)ln([Zn2+]/[Cu2+]).',
    hypothesisAr: 'قوة الخلية القياسية E0 = ١٫١٠ فولت تنخفض بزيادة تركيز أيونات المصعد [Zn2+] وتزداد بزيادة تركيز أيونات المهبط [Cu2+] وفق معادلة نيرنست.',
    ...
    apparatusAr: ['لوح خارصين (مصعد) مغمور في محلول كبريتات الخارصين', 'لوح نحاس (مهبط) مغمور في محلول كبريتات النحاس', 'قنطرة ملحية زجاجية بها هلام أجار ونترات بوتاسيوم KNO3', 'فولتميتر رقمي عالي الدقة', 'دائرة توصيل خارجية ومصباح إضاءة'],
    stepsEn: [
      'Assemble Daniell cell with 1.0 M Zn2+ and 1.0 M Cu2+ at standard 25 C (298.15 K).',
      'Measure standard electromotive force E0_cell on the voltmeter (1.10 V).',
      'Increase anode [Zn2+] to 2.0 M and observe EMF drop due to backward Le Chatelier shift.',
      'Increase cathode [Cu2+] to 2.0 M and observe positive rebound in cell voltage.',
      'Verify calculated Gibbs free energy change Delta G is negative, confirming spontaneous reaction.',
    ],
    stepsAr: [
      'ركّب خلية دانيال بمحلولي Zn2+ و Cu2+ بتركيز ١٫٠ مولار عند درجة حرارة ٢٥ مئوية القياسية.',
      'قِس القوة الدافعة الكهربية القياسية E0_cell على شاشة الفولتميتر (١٫١٠ فولت).',
      'زد تركيز أيونات المصعد [Zn2+] إلى ٢٫٠ مولار ولاحظ انخفاض الجهد طبقاً لقاعدة لوشاتيليه.',
      'زد تركيز أيونات المهبط [Cu2+] إلى ٢٫٠ مولار ولاحظ ارتفاع الجهد الناتج من الخلية.',
      'تحقق من أن التغير في طاقة جيبس الحرة Delta G سالب، مما يؤكد تلقائية التفاعل الكيميائي.',
    ],
  ...
  },
  {
    ...
    hypothesisAr: 'أكسدة مول واحد من الجلوكوز هوائياً تنتج ٣٢ جزيء ATP عبر الميتوكوندريا، بينما ينتج التخمر اللاهوائي جزيئين ATP فقط.',
    apparatusEn: ['Mitochondrial chemiosmosis simulator', 'Oxygen flow rate controller', 'Substrate glucose mole regulator', 'ATP synthase rotary motor visualizer', 'Respirometer gas volume analyzer'],
    ...
    stepsEn: [
      'Input 1.0 mole of glucose under full aerobic conditions (O2 available).',
      ...
      'Observe rapid shutdown of Krebs cycle and electron transport, yielding only 2 net ATP via glycolysis and lactate/ethanol formation.',
    ],
    stepsAr: [
      'أدخل ١٫٠ مول من الجلوكوز في وجود وفرة من غاز الأكسجين (تنفس هوائي كامل).',
      'تتبع المراحل الأربعة: انشطار الجلوكوز (٢ ATP)، تفاعل الربط، دورة كريبس، وسلسلة نقل الإلكترون.',
      'سجّل الحصيلة النهائية للطاقة (٣٢ جزيء ATP لكل مول جلوكوز) وانطلاق ٦ مولات من غاز CO2.',
      'أغلق مصدر الأكسجين لمحاكاة الإجهاد العضلي أو التخمر الكحولي في فطر الخميرة.',
      'لاحظ توقف دورة كريبس وسلسلة النقل، واقتصار الناتج على ٢ ATP فقط مع تراكم حمض اللاكتيك أو الإيثانول.',
    ],
  ...
  },
  {
    ...
    hypothesisAr: 'تفرز خلايا بيتا بجزر لانجرهانز الأنسولين عند ارتفاع السكر لتخزين الجليكوجين، بينما تفرز خلايا ألفا الجلوكاجون عند انخفاضه لتحفيز تحلل الجليكوجين.',
    ...
    stepsEn: [
      ...
      'Administer a 75g postprandial glucose spike to reach 160 mg/dL.',
    ...
    stepsAr: [
      'اضبط مستوى سكر الجلوكوز الصائم عند المستوى الطبيعي ٩٠ مجم/ديسيلتر.',
      'أدخل جرعة جلوكوز ٧٥ جم لمحاكاة تناول وجبة وارتفاع السكر إلى ١٦٠ مجم/ديسيلتر.',
      'لاحظ استجابة خلايا بيتا الفورية بإفراز الأنسولين لتحفيز نواقل الجلوكوز ودخوله الخلايا.',
      'تتبع بناء الجليكوجين في الكبد والعضلات وانخفاض السكر للمعدل الطبيعي (٨٠-١٢٠ مجم/ديسيلتر).',
      'حاكي حالة الصيام المطول؛ ولاحظ إفراز خلايا ألفا للجلوكاجون لتحويل الجليكوجين الكبدي إلى جلوكوز.',
    ],
  ...
  },
  {
    ...
    hypothesisAr: 'يحفز هرمونا FSH وLH النخاميان نضج حويصلة جراف والتبويض في اليوم الـ١٤، محكومين بالتغذية الراجعة للإستروجين والبروجسترون وتغيرات بطانة الرحم.',
    ...
    apparatusAr: ['مؤقت دورة الطمث التفاعلي (٢٨ يوماً)', 'راسم منحنيات الهرمونات النخامية (FSH وLH)', 'ماسح نضج حويصلة جراف وتكوين الجسم الأصفر', 'محاكي سمك وإمداد بطانة الرحم الدموي'],
    stepsEn: [
  ...
  },
  {
    ...
    hypothesisAr: 'تُنشط الخلايا البلعمية العارضة للأنتيجين عبر MHC خلايا Th المساعدة، التي تفرز السيتوكينات لتفعيل الخلايا البائية لإنتاج الأجسام المضادة وخلايا Tc القاتلة لإفراز البيرفورين.',
    ...
    apparatusAr: ['غرفة بلعمة ومعالجة الأنتيجين بالخلايا البلعمية', 'وحدة قياس مستقبلات التوافق النسيجي MHC والـ TCR', 'موزع إنترلوكينات وسيتوكينات الخلايا التائية المساعدة CD4+', 'مقياس إفراز البيرفورين والسموم اللمفاوية للخلايا التائية القاتلة CD8+'],
    stepsEn: [
    ...
    stepsAr: [
      ...
      'اربط الخلية التائية المساعدة Th بالمركب عبر مستقبل TCR، وشاهد إفراز الإنترلوكينات المنشطة.',
      'تتبع المسار الأول (المناعة الخلطية): تنشيط الخلايا البائية للتمايز لخلايا بلازمية تنتج أجساماً مضادة وخلايا ذاكرة.',
      'تتبع المسار الثاني (المناعة الخلوية): إفراز السيتوكينات لتنشيط الخلايا التائية القاتلة Tc والخلايا القاتلة الطبيعية NK.',
      'شاهد التحام خلية Tc بالخلية المصابة وإفراز بروتين البيرفورين لتثقيب غشائها والسموم اللمفاوية لتحفيز الموت المبرمج.',
    ],
  ...
  },
  {
    ...
    hypothesisAr: 'مجموع مربعات جيوب تمام الاتجاه لأي متجه في الفراغ ثلاثي الأبعاد يساوي دائماً الواحد الصحيح.',
    ...
    stepsEn: [
      'Set direction angle with X-axis alpha = 45 deg and with Y-axis beta = 60 deg.',
      'Calculate theoretical angle with Z-axis gamma using direction cosines identity.',
      'Verify that cos^2(45) + cos^2(60) + cos^2(gamma) = 0.5 + 0.25 + 0.25 = 1.0 (gamma = 60 deg).',
    ...
    stepsAr: [
      'اضبط زاوية الاتجاه مع محور السينات alpha = ٤٥ درجة، ومع محور الصادات beta = ٦٠ درجة.',
      'احسب نظرياً الزاوية مع محور العينات gamma باستخدام متطابقة جيوب تمام الاتجاه.',
      'تحقق من أن cos²(45) + cos²(60) + cos²(gamma) = ٠٫٥ + ٠٫٢٥ + ٠٫٢٥ = ١٫٠ (gamma = ٦٠ درجة).',
