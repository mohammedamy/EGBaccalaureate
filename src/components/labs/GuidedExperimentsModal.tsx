...
const EXPERIMENTS: GuidedExperiment[] = [
  ...
  },
  {
    ...
    gradeEn: 'Grade 12 Physics - Electromagnetic Induction (Chapter 3)',
    gradeAr: 'الصف الثالث الثانوي - الحث الكهرومغناطيسي والمولدات الكهربية (الفصل الثالث)',
    ...
    apparatusAr: [
      'مولد دينامو تفاعلي ثلاثي الأبعاد مع قطبين مغناطيسيين مقعرين',
      'حلقتي انزلاق نحاسيتين وفرشاتين من الجرافيت الكربوني (وضع التيار المتردد)',
      'أسطوانة معدنية مشقوقة طولياً إلى نصفين معزولين (وضع التيار موحد الاتجاه)',
      'راسم ذبذبات رقمي افتراضي (أوسيلوسكوب) لعرض إشارة الجهد الآنية والفعالة',
      'جهاز السكتين النحاسيتين وسلك متحرك ومصباح حمل لقياس القوة الدافعة الحركية',
      'ملف لولبي أسطواني نحاسي ومغناطيس مستقيم وجلفانومتر حساس صفره في المنتصف',
    ],
    stepsEn: [
      'Mount the rectangular armature coil within the concave magnetic poles (B = 0.8 T).',
  ...
  },
  {
    ...
    gradeAr: 'الصف الثالث الثانوي - الفيزياء الحديثة: الأطياف الذرية والأشعة السينية (الفصل السادس)',
  ...
  },
  {
    ...
    hypothesisEn: 'The continuous volume of revolution $V = \\pi \\int_a^b [f(x)]^2 dx$ equals the infinite limit of discrete cylindrical Riemann discs $V_N = \\sum_{i=1}^N \\pi [f(x_i)]^2 \\Delta x$, with numerical error vanishing as $N \\to \\infty$.',
    hypothesisAr: 'الحجم المتصل للجسم الدوراني $V = \\pi \\int_a^b [f(x)]^2 dx$ يمثل النهاية الدقيقة لتجميع الأقراص الأسطوانية المتتالية $V_N = \\sum_{i=1}^N \\pi [f(x_i)]^2 \\Delta x$، وينعدم الخطأ الحسابي تدريجياً مع زيادة عدد الأقراص $N \\to \\infty$.',
    apparatusEn: [
      'Interactive 2.5D Isometric Solids of Revolution Canvas',
      'Continuous 360-degree rotational angle controller (θ = 30° to 360°)',
      'Riemann disc slicing count slider (N = 4 to 60)',
      'Standard curriculum function presets (Paraboloid, Cone, Hemisphere, Vase, Hollow Washer)',
    ...
    apparatusAr: [
      ...
      'محدد زاوية الدوران المحوري المتصلة (من 30° إلى 360° كاملة)',
    ...
    stepsEn: [
      'Select Paraboloid preset $f(x) = \\sqrt{x}$ over interval $[a, b] = [0, 4]$.',
      'Compute theoretical analytical volume: $V_{\\text{exact}} = \\pi \\int_0^4 x dx = \\pi [\\frac{x^2}{2}]_0^4 = 8\\pi \\approx 25.1327$.',
      'Set disc count to coarse partition $N = 4$; note cylinder width $\\Delta x = 1.0$ and observe discrete step approximation error ($\approx 12.5\\%$).',
      'Increase disc count to $N = 12, 24$, and $60$; observe error decreasing rapidly below $0.5\\%$.',
      'Drag rotation sweep angle from $90^\\circ$ to $360^\\circ$; inspect cross-sectional slice geometry and circular profile formation.',
      'Switch to Washer hollow preset $f_{\\text{outer}} = \\sqrt{x}$, $g_{\\text{inner}} = \\frac{x}{2}$; verify washer subtraction volume formula.',
    ],
    stepsAr: [
      'اختر نموذج المجسم المكافئ الدوراني $f(x) = \\sqrt{x}$ على الفترة $[a, b] = [0, 4]$.',
      'احسب الحجم النظري التحليلي بالتكامل: $V_{\\text{exact}} = \\pi \\int_0^4 x dx = 8\\pi \\approx 25.1327$ وحدة مكعبة.',
      'اضبط عدد الأقراص على تجزيء تقريبي خشن $N = 4$؛ ولاحظ سمك القرص $\\Delta x = 1.0$ وسجل نسبة الخطأ التجزيئي (نحو $12.5\\%$).',
      'زد عدد الأقراص تدريجياً إلى $N = 12$ و $24$ ثم $60$ قرصاً؛ ولاحظ هبوط نسبة الخطأ إلى أقل من $0.5\\%$.',
      'اسحب زاوية الدوران من $90^\\circ$ حتى $360^\\circ$ كاملة؛ وتأمل تشكل الجسم المصمت هندسياً وتطابق دائرية المقاطع.',
      'انتقل لنموذج الحلقة المفرغة (Washer) بين دالتين؛ وتحقق من طرح حجم التجويف الداخلي من الحجم الخارجي.',
    ],
    formula: 'V = \\pi \\int_a^b [f(x)]^2 \\, dx, \\quad V_N = \\sum_{i=1}^N \\pi [f(x_i^*)]^2 \\Delta x, \\quad \\lim_{N \\to \\infty} V_N = V',
    expectedConclusionEn: 'Riemann cylindrical sum converges rigorously to the definite integral of revolution, proving that continuous 3D solids decompose into infinitesimally thin discs.',
    expectedConclusionAr: 'يتقارب مجموع ريمان للأقراص الأسطوانية بدقة متناهية نحو قيمة التكامل المحدد، مما يثبت تجزؤ المجسمات الدورانية ثلاثية الأبعاد إلى شرائح قرصية لامتناهية الصغر.',
  },
  {
    id: 'math-exp-7',
    lab: 'math',
    ...
    hypothesisEn: 'A conic section is the locus of points whose ratio of distance from focus to directrix is constant eccentricity $e$; parabolic reflectors collimate focal rays into parallel beams, while elliptic mirrors focus all rays from one focus to the other.',
    hypothesisAr: 'القطع المخروطي هو المحل الهندسي لنقطة تتحرك بحيث تكون النسبة بين بعدها عن البؤرة إلى بعدها عن الدليل ثابتة وتساوي الاختلاف المركزي $e$؛ وتعكس المرايا المكافئة الأشعة الصادرة من البؤرة في حزمة متوازية تماماً، بينما تعكس المرايا الناقصة كل شعاع من إحدى البؤرتين ليمر بالبؤرة الأخرى.',
    apparatusEn: [
      'Unified Conic Section Analytic Coordinate Canvas',
      'Eccentricity controller ($e = 0$ Circle, $e < 1$ Ellipse, $e = 1$ Parabola, $e > 1$ Hyperbola)',
      'Dynamic Focal point ($F$) and Directrix line visualizer',
      'Animated Optical Ray-Tracing Engine with law of specular reflection ($\\theta_i = \\theta_r$)',
      'Curvature radius and asymptote angle analyzer',
    ],
    apparatusAr: [
      'لوحة تحليل القطوع المخروطية الموحدة في المستوى الديكارتي',
      'متحكم الاختلاف المركزي ($e = 0$ دائرة، $e < 1$ قطع ناقص، $e = 1$ قطع مكافئ، $e > 1$ قطع زائد)',
      'محدد موقع البؤرة ($F$) وخط الدليل العمودي مع قياس المسافات اللحظية',
      'محاكي تتبع الأشعة الضوئية البصرية مع تحقيق قانون الانعكاس المنتظم (زاوية السقوط = زاوية الانعكاس)',
      'أداة حساب نصف قطر الانحناء وخطوط التقارب المحاذية للقطع الزائد',
    ],
    stepsEn: [
      'Select Parabola ($e = 1.0, y^2 = 4ax$ with $a = 2.0$). Verify focus at $(2, 0)$ and directrix line at $x = -2$.',
      'Pick point $P(2, 4)$ on curve; verify $PF = \\sqrt{(2-2)^2 + 4^2} = 4$ and distance to directrix $PD = 2 - (-2) = 4$, confirming $PF/PD = 1.0$.',
      'Turn on Ray-Tracing animation; emit rays from focal point $F(2, 0)$ hitting parabolic curve.',
      'Verify that all reflected rays emerge strictly parallel to the primary symmetry axis (optical collimation).',
      'Switch to Ellipse ($e = 0.6$); emit rays from Focus 1 ($F_1$) and observe all rays converging exactly at Focus 2 ($F_2$) after boundary reflection (whispering gallery).',
      'Switch to Hyperbola ($e = 1.5$); observe asymptotic divergence and virtual focal ray trajectory.',
    ],
    stepsAr: [
      'اختر القطع المكافئ ($e = 1.0$ والمعادلة $y^2 = 4ax$ مع $a = 2.0$)؛ وتأكد من موقع البؤرة عند $(2, 0)$ والدليل عند $x = -2$.',
      'اختر نقطة على المنحنى $P(2, 4)$؛ تحقق من أن بعدها عن البؤرة $PF = 4$ وبعدها عن الدليل $PD = 4$، مما يؤكد أن النسبة $PF/PD = 1.0$.',
      'شغّل محاكاة تتبع الأشعة الضوئية؛ واطلق أشعة ضوئية من البؤرة $F(2, 0)$ نحو جدار القطع المكافئ.',
      'تحقق من ارتداد جميع الأشعة المنعكسة في خطوط متوازية تماماً موازية لمحور التماثل (مبدأ كشافات الإضاءة والتلسكوبات).',
      'انتقل إلى القطع الناقص ($e = 0.6$)؛ وأطلق أشعة من البؤرة الأولى $F_1$ ولاحظ تجمعها التام عند البؤرة الثانية $F_2$ بعد الانعكاس (خاصية القبة الصوتية الهمسية).',
      'انتقل إلى القطع الزائد ($e = 1.5$)؛ وافحص انفراج المسارات وتطابقها مع خطوط التقارب المائلة.',
    ],
    formula: '\\frac{PF}{PD} = e, \\quad y^2 = 4ax \\; (e=1), \\quad \\frac{x^2}{a^2} + \\frac{y^2}{b^2} = 1 \\; (e < 1), \\quad \\frac{x^2}{a^2} - \\frac{y^2}{b^2} = 1 \\; (e > 1)',
    expectedConclusionEn: 'Conic reflection laws demonstrate the deep duality between analytic eccentricity and optical focal convergence, validating telescope and acoustic architecture principles.',
    expectedConclusionAr: 'تبرهن الخواص البصرية للقطوع المخروطية الارتباط الوثيق بين قيمة الاختلاف المركزي وتجمع الأشعة في البؤر، وهو الأساس الرياضي لتصميم هوائيات الأقمار الصناعية والتلسكوبات.',
  },
  {
    id: 'math-exp-8',
    lab: 'math',
    ...
    hypothesisEn: 'Any periodic piecewise-continuous waveform decomposes into a unique infinite series of sinusoidal harmonics; truncation at $N$ harmonics produces an intrinsic $\\approx 8.95\\%$ overshoot at jump discontinuities (Gibbs phenomenon).',
    hypothesisAr: 'يمكن تفكيك أي دالة دورية متصلة على فترات إلى مجموع لا نهائي من التوافقيات الجيبية البسيطة؛ وينتج عن قطع المتسلسلة عند عدد محدود $N$ قفزة زائدة ثابتة تبلغ $\\approx 8.95\\%$ عند نقاط عدم الاتصال (ظاهرة جيبس).',
    apparatusEn: [
      'Rotating Epicycle Phasor Circles Canvas (complex rotational vectors)',
      'Real-time Scrolling Time-Domain Synthesized Waveform Display',
      'Interactive Harmonic Count controller ($N = 1$ to $35$ harmonics)',
      'Waveform shape presets (Square wave, Sawtooth wave, Triangle wave, Full-wave Rectified)',
      'Real-time Web Audio API frequency tone generator with harmonic timbre synthesis',
      'Discrete Fourier amplitude bar spectrum chart',
    ],
    apparatusAr: [
      'لوحة فلكيات التدوير الدوارة (متجهات التردد الدائرية Epicycles)',
      'شاشة عرض الدالة الزمنية المتزامنة للموجة الناتجة عن التركيب',
      'متحكم عدد التوافقيات التفاعلي ($N$ من 1 إلى 35 توافقية فردية وزوجية)',
      'نماذج الموجات القياسية (الموجة المربعة، سن المنشار، المثلثية، والموجة المقومة تقويماً كاملاً)',
      'مولد نغمات صوتية حي باستخدام Web Audio API لسماع تغير جرس الصوت مع إضافة التوافقيات',
      'مخطط أعمدة الترددات الطيفي لسعات التوافقية ($b_n$ مقابل التردد)',
    ],
    stepsEn: [
      'Select Square Waveform ($f(t) = \\text{sgn}(\\sin t)$); note theoretical coefficients $b_n = \\frac{4}{\\pi n}$ for odd $n$.',
      'Set $N = 1$ harmonic (pure fundamental sine wave); enable audio to hear smooth $220\\text{ Hz}$ tone.',
      'Increase to $N = 3, 5, 9$ harmonics; observe corner squaring in time-domain and audible harmonic enrichment (brighter timbre).',
      'Increase to $N = 35$ harmonics; observe high-frequency ringing near jump discontinuities and measure peak overshoot $(\\approx 1.1789 \\times \\text{amplitude} \\implies 8.95\\%)$.',
      'Switch to Triangle wave ($b_n \\propto \\frac{1}{n^2}$); observe rapid convergence without Gibbs overshoot due to wave continuity.',
      'Inspect Fourier spectrum bar graph; verify inverse power-law roll-off rates of different waveform families.',
    ],
    stepsAr: [
      'اختر الموجة المربعة ($f(t) = \\text{sgn}(\\sin t)$)؛ وافحص المعاملات النظرية $b_n = \\frac{4}{\\pi n}$ للتوافقيات الفردية فقط.',
      'اضبط عدد التوافقيات على $N = 1$ (النغمة التوافقية الأساسية الخالصة)؛ وشغّل الصوت لسماع نغمة جيبية نقية بتردد $220\\text{ Hz}$.',
      'زد التوافقيات إلى $N = 3$ و $5$ ثم $9$؛ ولاحظ استقامة جوانب الموجة وسماع صوت أكثر خشونة وثراء نغمياً.',
      'ارفع التوافقيات إلى $N = 35$؛ ولاحظ تذبذب الحواف عند نقاط الانفصال وقس الارتفاع الزائد للقمة ($\approx 8.95\\%$ ظاهرة جيبس).',
      'انتقل للموجة المثلثية ذات المعاملات المتناقصة بسرعة ($b_n \\propto 1/n^2$)؛ ولاحظ سرعة التقارب وغياب ظاهرة جيبس نظراً لاتصال الدالة.',
      'افحص مخطط طيف الترددات؛ وتأكد من هبوط سعة الترددات العالية وفق القانون الرياضي لكل شكل موجي.',
    ],
    formula: 'f(t) = \\frac{a_0}{2} + \\sum_{n=1}^\\infty \\left[ a_n \\cos(nt) + b_n \\sin(nt) \\right], \\quad b_n = \\frac{4}{\\pi n} \\; (\\text{odd } n)',
    expectedConclusionEn: 'Fourier series successfully constructs arbitrary geometric waveforms via trigonometric superpositions, confirming harmonic synthesis and the mathematical persistence of the Gibbs overshoot.',
    expectedConclusionAr: 'تثبت متسلسلات فورييه إمكانية بناء أي موجة هندسية عبر تراكم نغمات جيبية متناغمة، مؤكدة صحة التحليل الطيفي وظاهرة جيبس الحتمية عند القفزات غير المتصلة.',
  },
  {
    id: 'math-exp-9',
    lab: 'math',
    ...
    hypothesisEn: 'A first-order differential equation $\\frac{dy}{dx} = f(x, y)$ defines a geometric slope field; Euler polygonal steps $y_{n+1} = y_n + h f(x_n, y_n)$ converge to the true continuous integral curve as step size $h \\to 0$.',
    hypothesisAr: 'المعادلة التفاضلية من الرتبة الأولى $\\frac{dy}{dx} = f(x, y)$ تمثل هندسياً حقلاً لاتجاهات الميول؛ وتتقارب المضلعات المتتالية لطريقة أويلر العددية $y_{n+1} = y_n + h f(x_n, y_n)$ نحو منحنى الحل التحليلي الحقيقي كلما صغرت خطوة التقريب $h \\to 0$.',
    apparatusEn: [
      'Dense 2D Slope Field Direction Grid with normalized tangent line segments',
      'Interactive Initial Condition Coordinate Selector $(x_0, y_0)$ with click-to-place canvas',
      'Continuous 4th-order Runge-Kutta integral streamline solver',
      'Discrete Euler numerical step visualizer with customizable step size $h$ ($0.05$ to $0.5$)',
      'Curriculum differential equation presets (Logistic Growth, Harmonic Slope, Newton Cooling, Linear Separable)',
      'Real-time numerical step comparison table with truncation error analysis',
    ],
    apparatusAr: [
      'شبكة حقول الميول ثنائية الأبعاد لأسهم الاتجاه المماسي المعايرة',
      'محدد الشروط الابتدائية التفاعلي $(x_0, y_0)$ مع إمكانية النقر المباشر على اللوحة',
      'محرك حل المنحنى التكاملي المتصل بطريقة رونج-كوتا من الرتبة الرابعة',
      'أداة تتبع خطوات أويلر العددية المضلعة مع التحكم في طول الخطوة $h$ (من $0.05$ إلى $0.5$)',
      'نماذج المعادلات التفاضلية المقررة (النمو اللوجستي المقيد، الميول التوافقية، تبريد نيوتن، والدوال القابلة للفصل)',
      'جدول مقارنة تفصيلي للخطوات العددية وقيم الخطأ المتراكم (Global Truncation Error)',
    ],
    stepsEn: [
      'Select Logistic Growth preset $\\frac{dy}{dx} = 0.8 y (1 - \\frac{y}{4})$; observe slope segments leveling off at carrying capacity $y = 4$ and $y = 0$.',
      'Click canvas at initial point $(x_0, y_0) = (0, 0.5)$; observe S-shaped continuous logistic sigmoid curve asymptotic to $y = 4$.',
      'Set Euler step size to coarse value $h = 0.4$; trace polygon vertices and record numerical deviations from true curve.',
      'Reduce step size to $h = 0.1$ and $h = 0.05$; verify that Euler polygonal chain hugs the exact solution closely.',
      'Switch to Harmonic Slope $\\frac{dy}{dx} = -\\frac{x}{y}$; verify concentric circular integral curves ($x^2 + y^2 = C$).',
      'Analyze truncation error accumulation: observe that global error scales proportionally with step size $\\mathcal{O}(h)$.',
    ],
    stepsAr: [
      'اختر معادلة النمو اللوجستي $\\frac{dy}{dx} = 0.8 y (1 - \\frac{y}{4})$؛ ولاحظ أفقية أسهم الميول عند السعة القصوى $y = 4$ وعند خط الأساس $y = 0$.',
      'انقر على لوحة الإحداثيات عند النقطة الابتدائية $(x_0, y_0) = (0, 0.5)$؛ ولاحظ رسم المنحنى اللوجستي المتصل المقارب للخط $y = 4$.',
      'اضبط خطوة أويلر على قيمة تقريبية كبيرة $h = 0.4$؛ وتتبع رؤوس المضلع وسجل مقدار الانحراف عن المنحنى الدقيق.',
      'صغّر خطوة التقريب إلى $h = 0.1$ ثم $h = 0.05$؛ وتحقق من انطباق مسار أويلر شبه التام على المنحنى التحليلي.',
      'انتقل لمعادلة الميول التوافقية الدائرية $\\frac{dy}{dx} = -\\frac{x}{y}$؛ وتأكد من تشكل مسارات دائرية مغلقة متحدة المركز ($x^2 + y^2 = C$).',
      'حلل الخطأ المتراكم في جدول القراءات؛ واستنتج تناسب الخطأ الإجمالي طردياً مع طول خطوة التقريب $\\mathcal{O}(h)$.',
    ],
    formula: '\\frac{dy}{dx} = f(x, y), \\quad y_{n+1} = y_n + h f(x_n, y_n), \\quad E_{\\text{global}} \\propto \\mathcal{O}(h)',
    expectedConclusionEn: 'Slope fields geometrically visualize differential solutions without explicit integration, while Euler method proves how numerical stepping approximates continuous dynamics.',
    expectedConclusionAr: 'تكشف حقول الميول السلوك الهندسي لحلول المعادلات التفاضلية دون الحاجة للتكامل الجبري المباشر، وتؤكد خوارزمية أويلر كيفية محاكاة الظواهر الديناميكية المتصلة عددياً.',
  },
  {
    id: 'math-exp-10',
    lab: 'math',
    ...
    hypothesisEn: 'In linear programming with convex polygonal feasible region $S$, the maximum or minimum of linear objective function $Z = c_1 x + c_2 y$ occurs exclusively at one or more boundary extreme vertices (Corner Point Theorem).',
    hypothesisAr: 'في مسائل البرمجة الخطية ذات منطقة الحلول المحدبة المضلعة $S$، تقع القيمة العظمى أو الصغرى لدالة الهدف الخطية $Z = c_1 x + c_2 y$ حتماً عند أحد رؤوس المضلع (نظرية النقطة الرأسية - Corner Point Theorem).',
    apparatusEn: [
      'Interactive 2D Cartesian Linear Programming Optimization Canvas',
      'Half-plane linear inequality boundary shading engine ($a_i x + b_i y \\le d_i$)',
      'Automated convex feasible polygon clipping and vertex intersection detector',
      'Interactive Objective Function line controller ($Z = c_1 x + c_2 y$) with animated profit sweep',
      'Real-world curriculum scenario presets (Manufacturing profit maximization, Crop land allocation, Dietary cost minimization)',
      'Corner Point vertex evaluation data table with optimal solution highlighting',
    ],
    apparatusAr: [
      'لوحة استكشاف البرمجة الخطية والحلول المثلى في المستوى ثنائي الأبعاد',
      'محرك تظليل أنصاف المستويات للمتباينات الخطية ($a_i x + b_i y \\le d_i$)',
      'محدد مضلع منطقة الحلول الممكنة المحدبة وحساب إحداثيات نقاط التقاطع تلقائياً',
      'متحكم خط دالة الهدف ($Z = c_1 x + c_2 y$) مع خاصية المسح الديناميكي للربح',
      'نماذج سيناريوهات واقعية مقررة (تعظيم ربح مصنع أثاث، توزيع زراعة المحاصيل، وأدنى تكلفة للوجبة الغذائية)',
      'جدول تقييم رؤوس الحلول الممكنة وتحديد النقطة المثلى تلقائياً مع نسبة الإنجاز',
    ],
    stepsEn: [
      'Select Furniture Manufacturing preset with constraints: $2x + y \\le 100$ (Carpentry), $x + 2y \\le 80$ (Finishing), and non-negativity $x, y \\ge 0$.',
      'Observe shaded convex quadrilateral feasible polygon with 4 corner vertices: $(0, 0), (50, 0), (40, 20), (0, 40)$.',
      'Set objective function coefficients to profit parameters $c_1 = 40, c_2 = 30$ ($Z = 40x + 30y$).',
      'Evaluate $Z$ at all 4 vertices: $Z(0, 0) = 0$, $Z(50, 0) = 2000$, $Z(0, 40) = 1200$, $Z(40, 20) = 40(40) + 30(20) = 2200$.',
      'Animate objective line sweep; observe the parallel profit lines sweeping outwards until exiting the feasible set precisely at vertex $(40, 20)$.',
      'Modify constraint sliders; observe dynamic polygon vertex recalculation and real-time optimal vertex reassignment.',
    ],
    stepsAr: [
      'اختر نموذج تصنيع الأثاث مع قيود الموارد: $2x + y \\le 100$ (النجارة)، $x + 2y \\le 80$ (التشطيب)، وقيد عدم السالبية $x, y \\ge 0$.',
      'تأمل مضلع منطقة الحلول الممكنة المحدب المظلل برؤوسه الأربعة: $(0, 0)$ و $(50, 0)$ و $(40, 20)$ و $(0, 40)$.',
      'اضبط معاملات دالة الهدف على عوامل الربح: $c_1 = 40$ و $c_2 = 30$ لتكون دالة الربح $Z = 40x + 30y$.',
      'احسب قيمة دالة الهدف عند كل رأس: $Z(0, 0) = 0$، $Z(50, 0) = 2000$، $Z(0, 40) = 1200$، و $Z(40, 20) = 2200$ (الربح الأقصى).',
      'شغّل المسح التفاعلي لمستوى خط الهدف؛ ولاحظ تحرك الخط موازياً لنفسه حتى يلامس المضلع في آخر نقطة خروج عند الرأس $(40, 20)$.',
      'غير قيم معاملات المتباينات؛ ولاحظ إعادة حساب الرؤوس فورياً وانتقال نقطة الحل الأمثل بحسب الحدود الجديدة.',
    ],
    formula: '\\max Z = c_1 x + c_2 y \\quad \\text{subject to} \\quad A \\mathbf{x} \\le \\mathbf{b}, \\; \\mathbf{x} \\ge 0',
    expectedConclusionEn: 'The Corner Point Theorem is validated: linear objective extrema strictly coincide with boundary vertices, providing an infallible mathematical strategy for industrial resource allocation.',
    expectedConclusionAr: 'تتحقق نظرية النقاط الرأسية تماماً: حيث تقع القيمة العظمى للهدف دائماً عند أحد رؤوس مضلع القيود، مما يوفر أداة رياضية قاطعة لاتخاذ القرارات الاقتصادية والصناعية المثلى.',
  },
];
