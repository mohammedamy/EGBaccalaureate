        ...
        <div className="space-y-4">
          ...
          {systemMode === 'xray_coolidge' && (
            ...
            >
              ...
              {/* Duane-Hunt Quantitative Analysis & Formula HUD */}
              <div
              ...
              >
                <div className="flex items-center justify-between">
                  <span className={`font-bold flex items-center gap-1.5 ${isLight ? 'text-purple-900' : 'text-purple-300'}`}>
                    ...
                    <span>{isArabic ? 'قانون دوين-هنت (Duane-Hunt Law):' : 'Duane-Hunt Quantitative Metrics:'}</span>
                  </span>
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px]">
                  <div className={`p-2 rounded-lg border ${isLight ? 'bg-white border-purple-100' : 'bg-stone-900/90 border-purple-900/50'}`}>
                    <span className="block text-[10px] opacity-75">{isArabic ? 'الطول الموجي الأدنى:' : 'Minimum Wavelength:'}</span>
                    <span className="font-mono font-black text-purple-500 text-xs">
                      λ_min = {xrayLambdaMinNm.toFixed(3)} nm
                    </span>
                    <span className="block text-[9px] text-stone-500">
                      = {(xrayLambdaMinNm * 10).toFixed(2)} Å
                    </span>
                  </div>

                  <div className={`p-2 rounded-lg border ${isLight ? 'bg-white border-purple-100' : 'bg-stone-900/90 border-purple-900/50'}`}>
                    <span className="block text-[10px] opacity-75">{isArabic ? 'التردد الأقصى للطيف المستمر:' : 'Maximum Continuous Frequency:'}</span>
                    <span className="font-mono font-black text-purple-500 text-xs">
                      ν_max = {( (3e8 / (xrayLambdaMinNm * 1e-9)) / 1e18 ).toFixed(2)} × 10¹⁸ Hz
                    </span>
                    <span className="block text-[9px] text-stone-500">
                      E_max = {xrayVoltageKv} keV
                    </span>
                  </div>
                </div>

                {/* Moseley Target Shift & Critical Voltage Check */}
                <div
                  className={`p-2.5 rounded-lg border space-y-1 text-[11px] ${
                    hasCharacteristicLines
                      ? isLight
                ...
                >
                  <div className="flex items-center justify-between font-bold text-xs">
                    <span>{isArabic ? 'قانون موزلي والطيف المميز:' : 'Moseley Law & Characteristic Peaks:'}</span>
                    <span
                    ...
                    >
                      {hasCharacteristicLines
                        ...
                        : isArabic
                        ? '✕ أقل من جهد الإثارة الحرج'
                        : '✕ Below Critical Voltage'}
                    </span>
                  </div>
                  ...
                  <div className="text-[10px] space-y-0.5">
                    <p>
                      {isArabic
                        ? `جهد الإثارة الحرج لمعدن ${currentTarget.elementAr} (Z=${currentTarget.z}): V_crit = ${currentTarget.criticalKv} kV.`
                        : `Critical excitation potential for ${currentTarget.elementEn} (Z=${currentTarget.z}): V_crit = ${currentTarget.criticalKv} kV.`}
                    </p>
                    {hasCharacteristicLines ? (
                      <p className="font-mono text-amber-500 font-bold">
                        K_α = {currentTarget.kAlphaNm.toFixed(3)} nm | K_β = {currentTarget.kBetaNm.toFixed(3)} nm
                      </p>
                    ) : (
                      <p className="text-rose-400">
                        {isArabic
                          ? `ارفع فرق الجهد إلى أعلى من ${currentTarget.criticalKv} kV لنزع إلكترون من المدارات الداخلية (K).`
                          : `Increase voltage above ${currentTarget.criticalKv} kV to eject K-shell inner electrons.`}
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div
                  className={`p-2 rounded-lg ${
                    isLight ? 'bg-purple-100/60 text-purple-950' : 'bg-purple-950/60 text-purple-200'
                  } text-[10px] leading-relaxed`}
                >
                  <span className="font-bold block text-purple-400 mb-0.5">
                    {isArabic ? '💡 فخاخ امتحانات الثانوية العامة:' : '💡 Ministerial Exam Master Rules:'}
                  </span>
                  <ul className="list-disc list-inside space-y-0.5">
                    <li>
                      {isArabic
                        ? 'λ_min (المستمر / الكبح) يعتمد فقط على فرق الجهد المعجل V وعكسي معه، ولا يتأثر بنوع مادة الهدف.'
                        : 'λ_min depends solely on accelerating voltage V (inverse), unaffected by target atomic number.'}
                    </li>
                    <li>
                      {isArabic
                        ? 'طول الخط المميز K_α يعتمد فقط على العدد الذري لمادة الهدف (عكسي مع Z²) ولا يتأثر إطلاقاً بفرق الجهد.'
                        : 'Characteristic wavelengths (K_α, K_β) depend strictly on target atomic number Z (inverse to Z²), invariant with tube voltage.'}
                    </li>
                  </ul>
                </div>
              </div>
