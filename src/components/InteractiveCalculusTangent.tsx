...
export const InteractiveCalculusTangent: React.FC<Props> = ({ lang }) => {
  ...
  useEffect(() => {
    ...
    // 5. Draw Tangent Line (Emerald/Green solid)
    ctx.strokeStyle = '#10b981';
    ctx.lineWidth = 2;
    ctx.beginPath();
    const tangentY1 = fx0 + exactDerivative * (-5 - pointX);
    const tangentY2 = fx0 + exactDerivative * (5 - pointX);
    ctx.moveTo(toCanvasX(-5), toCanvasY(tangentY1));
    ctx.lineTo(toCanvasX(5), toCanvasY(tangentY2));
    ctx.stroke();

    // 6. Draw Points A (x0, f(x0)) and B (x0+h, f(x0+h))
    // Point A
    ctx.fillStyle = '#facc15'; // Yellow point A
    ctx.beginPath();
  ...
  return (
    <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-4 sm:p-6 shadow-2xl space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-4">
        ...
        {/* Function selector */}
        <div className="flex flex-wrap items-center justify-center gap-1.5 sm:gap-2 bg-slate-950 p-1.5 sm:p-2 rounded-xl border border-slate-800 w-full sm:w-auto">
          <button
            onClick={() => setFuncType('cubic')}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${funcType === 'cubic' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-600/30' : 'text-slate-400 hover:text-white'}`}
          >
            f(x) = x³ - 3x
          </button>
          <button
            onClick={() => setFuncType('quadratic')}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${funcType === 'quadratic' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-600/30' : 'text-slate-400 hover:text-white'}`}
          >
            f(x) = 0.5x²
          </button>
          <button
            onClick={() => setFuncType('sine')}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${funcType === 'sine' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-600/30' : 'text-slate-400 hover:text-white'}`}
          >
            f(x) = 2 sin(x)
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 bg-slate-950 p-3.5 sm:p-4 rounded-xl border border-slate-800 flex flex-col items-center">
          <canvas ref={canvasRef} width={500} height={380} className="w-full max-w-[500px] aspect-[500/380] h-auto rounded-lg border border-slate-800/80" />
          <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-4 mt-3 text-xs">
            <span className="text-cyan-400 font-bold">Curve f(x)</span>
            <span className="text-amber-400 font-bold">Secant Line (h={stepH.toFixed(2)})</span>
            <span className="text-emerald-400 font-bold">Tangent Line (dy/dx)</span>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-3">
            <div>
              ...
              <input type="range" min="-2.5" max="2.5" step="0.1" value={pointX} onChange={(e) => setPointX(Number(e.target.value))} className="w-full accent-indigo-500" />
            </div>

            <div>
              <label className="text-xs text-amber-300 font-semibold flex justify-between">
                <span>{lang === 'ar' ? 'خطوة المقدار h (تئول للصفر):' : 'Step Increment h (h -> 0):'}</span>
                <span>{stepH.toFixed(2)}</span>
              </label>
              <input type="range" min="0.05" max="3" step="0.05" value={stepH} onChange={(e) => setStepH(Number(e.target.value))} className="w-full accent-amber-400" />
            </div>
          </div>
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-3 text-xs">
            ...
            <div className="bg-slate-900 p-3 rounded-lg space-y-1">
              <span className="text-slate-400 block">{lang === 'ar' ? 'ميل القاطع التجريبي:' : 'Approx Secant Slope:'}</span>
            ...
            <div className="bg-slate-900 p-3 rounded-lg space-y-1">
              <span className="text-emerald-400 block font-semibold">{lang === 'ar' ? 'ميل المماس (المشتقة الأولى dy/dx):' : 'Exact Derivative Slope (dy/dx):'}</span>
